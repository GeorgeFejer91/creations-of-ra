...
- `presentations/deck.js`, `view.js`: load/render the supplied image/video PPTX.
- `presentations/transport.js`: one fixed VDO.Ninja data-only publisher stream. The phone is the publisher/controller; every presentations page is a viewer. No camera/microphone.
...
`/presentations/` is always the viewer page. It shows the first slide and one QR when idle. The QR opens the hidden smartphone controller, which announces one fixed VDO.Ninja data-only stream. The first phone to claim that stream is the sole controller; VDO.Ninja rejects another live publisher using the same stream ID. There is no name form, approval dialog, viewer QR, viewer subpage or controls checkbox. While the phone is connected, every browser already on `/presentations/` and every browser arriving later follows the same slide/media/blackout state and the page becomes a viewport-filling presentation with no local controls. `Finish presentation`, controller page close, or controller disconnect ends the live state and viewers return to the preview plus QR.
